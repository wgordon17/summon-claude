...
- **Scribe agent session profile** — `--scribe-profile` internal flag, persistent `0-scribe` channel with reuse-or-create pattern, scribe-specific canvas template, scan timer via `SessionScheduler`, and hardened prompt injection defense with attack pattern examples and canary phrase
